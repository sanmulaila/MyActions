...
## 删除失效 Cookie 开关，默认是 0，表示关闭；设置为 1，表示开启
DEL_COOKIE="0"
...
## 调试模式开关，默认是 0，表示关闭；设置为 1，表示开启
DEBUG="1"

## 本脚本限制的最大线程数量
proc_num="7"

## 备份配置文件开关，默认是 1，表示开启；设置为 0，表示关闭。备份路径 /ql/config/bak/
BACKUP="1"
## 是否删除指定天数以前的备份文件开关，默认是 1，表示开启；设置为 0，表示关闭。删除路径 /ql/config/bak/
CLEANBAK="1"
## 定义删除指定天数以前的备份文件
CLEANBAK_DAYS="2"

## 定义 jcode 脚本导出的互助码模板样式（选填）
## 不填 使用 “按编号顺序互助模板”，Cookie 编号在前的优先助力
...
diy_help_rules (){
    case $1 in
        ...
        DreamFactory | JdFactory)
            ...
            ;;
        Jdzz | Joy)
            tmp_helptype="2"            # 京东赚赚和疯狂的 Joy 使用 “随机顺序互助模板”，本套脚本内账号间随机顺序助力，每次生成的顺序都不一致。
            ;;
        *)
...
##     d) 设定为 BreakHelpNum="4 9-14 15~18 19_21" 表示第 4 个账号、第 9 至 14 账号、第 15 至 18 账号、第 19 至 21 账号均不被助力。注意序号区间连接符仅支持 - ~ _；
## 不按示例填写可能引发报错。
...
## 定义是否自动安装或修复缺失的依赖，默认为 1，表示自动修复；留空或其他数值表示不修复。
FixDependType="1"
## 定义监控修复的依赖名称
package_name="canvas png-js date-fns axios crypto-js ts-md5 tslib @types/node dotenv typescript fs require tslib"

## 需组合的环境变量列表，env_name 需要和 var_name 一一对应，如何有新活动按照格式添加 (不懂勿动)
env_name=(
  FRUITSHARECODES
  PETSHARECODES
...
## name_js 为脚本文件名，如果使用 ql repo 命令拉取，文件名含有作者名
...
## 导出互助码的通用程序，$1：去掉后缀的脚本名称，$2：config.sh 中的后缀，$3：活动中文名称
export_codes_sub () {
    local task_name=$1
    local config_name=$2
    ...
    if cd $dir_log/$task_name &>/dev/null && [[ $(ls) ]]; then
        ...
        pt_pin_in_log=()
        code=()
        pt_pin_and_code=$(ls -r *.log | xargs awk -v var="的 $chinese_name 好友互助码" 'BEGIN {FS="[（ ）】]+"; OFS="&"} $3~var {print $2,$4}')
...
export_all_codes () {
    ...
    echo -e "\n#【`date +% X`】 从日志提取互助码，编号和配置文件中 Cookie 编号完全对应，如果为空就是所有日志中都没有。\n\n#【`date +% X`】 即使某个 MyXxx 变量未赋值，也可以将其变量名填在 ForOtherXxx 中，jtask 脚本会自动过滤空值。\n"
...
sed -i "1c ## 上次导入时间：$(date +% Y 年 % m 月 % d 日 \ % X)" $ShareCode_log
}
...
export_all_codes | perl -pe "{s | 京东种豆 | 种豆 |; s|crazyJoy 任务 | 疯狂的 JOY|}"
sleep 5
update_help

## 修改 curtinlv 入会领豆配置文件的参数
